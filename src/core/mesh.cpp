...
void Mesh::createIndexBuffer(VmaAllocator &allocator, VkQueue graphicsQueue, VkCommandPool commandPool)
{
    ...
    if (vmaCreateBuffer(allocator, &indexBufferInfo, &indexAllocInfo, &m_indexBuffer, &m_indexBufferAlloc, nullptr) !=
        VK_SUCCESS)
    {
        throw std::runtime_error("failed to create buffer with VMA!");
    }
...
void Mesh::createVertexBuffer(VmaAllocator &allocator, VkQueue graphicsQueue, VkCommandPool commandPool)
{
    ...
    if (vmaCreateBuffer(allocator, &vertexBufferInfo, &vertexAllocInfo, &m_vertexBuffer, &m_vertexBufferAlloc,
                        nullptr) != VK_SUCCESS)
    {
        throw std::runtime_error("failed to create buffer with VMA!");
    }
